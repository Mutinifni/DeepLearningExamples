...
MODEL_NAME=${MODEL_NAME:-jasper-onnx-ensemble}
...
MEASUREMENT_WINDOW=${MEASUREMENT_WINDOW:-10000}
...
echo "=== STARTING: perf client ${ARGS} --concurrency-range 2 ==="
set -x
docker run  -e DISPLAY=${DISPLAY}  --runtime nvidia --rm \
	      --privileged --net=host \
	      -v ${RESULT_DIR_H}:/results --name jasper-perf-client \
	      ${TRITON_CLIENT_CONTAINER_TAG}  perf_client $ARGS -f /results/${OUTPUT_FILE_CSV}_p1 --shape AUDIO_SIGNAL:${AUDIO_LENGTH} --concurrency-range 1 2>&1
set +x
...
#cat ${RESULT_DIR_H}/${OUTPUT_FILE_CSV}_p1 ${RESULT_DIR_H}/${OUTPUT_FILE_CSV}_p2 > ${RESULT_DIR_H}
cat ${RESULT_DIR_H}/${OUTPUT_FILE_CSV}_p1 > ${RESULT_DIR_H}/perf_analyzer.csv
